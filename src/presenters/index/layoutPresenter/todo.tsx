    ...
    return <>
        {
            props.todos?.map(todo => (
                <div key={todo.id}>
                    <input onChange={(e) => props.handleChangeTitle(todo.id, e.target.value)} value={todo.title}/>
                    <input onChange={(e) => props.handleChangeDescription(todo.id, e.target.value)} value={todo.description}/>
                    {(Array.isArray(props.todos) && props.todos?.length > 1) ?
                    <button onClick={() => props.handleDelete(todo.id)}>delete</button>
                    :
                    <button onClick={() => props.handleClear(todo.id)}>clear</button>}
                    <button onClick={() => props.handleClone(todo.id)}>clone</button>
                </div>
            ))
        }
    </>
